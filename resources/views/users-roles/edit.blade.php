        ...
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center">
                <h4 class="lead text-primary">
                    <i class="ph-users text-secondary"></i> Edit User: {{ $user->name }}
                </h4>
            ...
            <form action="{{ route('users-roles.update', $user->id) }}" method="POST">
                        ...
                        <select name="roles[]" class="form-control select2" multiple>
                            @foreach ($roles as $role)
                                <option value="{{ $role->id }}"
                                        {{ $user->roles->contains($role->id) || in_array($role->id, old('roles', [])) ? 'selected' : '' }}>
                                    {{ $role->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3 text-end">
                    <button type="submit" class="btn btn-primary" style="background-color: #1d5fc9; border-color: #1d5fc9;">
                        <i class="ph-check me-2"></i> Update User
                    </button>
                </div>
            </form>
        </div>
